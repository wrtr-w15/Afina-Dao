...
function smoothDampVec2(
  ...
  out.copy(current.clone().sub(change).add(change.add(temp).multiplyScalar(exp)));
...
export default function GridScan({
  ...
  useEffect(() => {
    ...
    if (enablePost) {
      ...
      const bloom = new BloomEffect({
        intensity: Math.max(0, bloomIntensity) || 0.6,
    ...
    const onResize = () => {
      renderer.setSize(container.clientWidth, container.clientHeight);
      (material.uniforms.iResolution.value as THREE.Vector3).set(
    ...
    const tick = () => {
      ...
      (material.uniforms.uSkew.value as THREE.Vector2).set(skew.x, skew.y);
  ...
  useEffect(() => {
    ...
    if (bloom && 'intensity' in bloom && typeof (bloom as { intensity: number }).intensity === 'number') {
      (bloom as { intensity: number }).intensity = Math.max(0, bloomIntensity);
    }
    const chroma = chromaRef.current;
    if (chroma && chroma.offset) chroma.offset.set(chromaticAberration, chromaticAberration);
  }, [
    lineThickness,
    linesColor,
    scanColor,
