...
/** Опции выдачи роли. sendNotification: отправить ЛС в Discord (при продлении не отправляем). */
export type GrantRoleOptions = { sendNotification?: boolean };

// Выдача роли пользователю
export async function grantRole(discordId: string, options?: GrantRoleOptions): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    // ЛС о выдаче доступа — только при первой выдаче (при покупке), не при продлении
    if (sendNotification) {
      try {
