...
Что-то пошло не так. Попробуйте ещё раз или обратитесь в поддержку.`,
  howToStartInCommunity: `📖 <b>Как начать работу в Сообществе</b>
...
export const messages = {
  ...
  howToStartInCommunity: async (notionEmail?: string | null): Promise<string> => {
    const notionEmailLine = notionEmail?.trim()
      ? ` (ваша почта: <code>${notionEmail.trim()}</code>)`
      : ' (проверьте почту, указанную при оплате)';
    const base = await text('howToStartInCommunity', { notionEmailLine });
    return base.replace(/\{\{notionEmailLine\}\}/g, notionEmailLine);
  },
